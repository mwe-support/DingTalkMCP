...
- The stable public surface contains exactly three business tools:
  ...
  3. `approval_inbox` owns bounded, read-only batch discovery of the authenticated user's pending or completed records and returns unique `processInstanceId` values for `approval_task`. Multiple task events for one instance must be aggregated into one record while retaining bounded `taskIds`, task counts, and decision evidence.
...
- A bounded OpenAPI refresh that returns `truncated=true` must also return an HMAC-authenticated opaque continuation cursor. Continuation calls stay in the same `approval_inbox` tool, bind the current user plus original time window/status/template set, reject tampering, retry any page whose candidate detail failed, and remain bounded per call until `truncated=false`.
...
- Do not make the DingTalk-hosted OA MCP gateway or DWS PAT store a runtime dependency. DingTalk Stream is permitted only as an outbound application-authenticated event-ingestion adapter; it is not an MCP transport and must not expose a second tool endpoint.
...
- `127.0.0.1:3000` is registered to the sole active production DingTalk backend. Any later candidate must use another separately verified free loopback port and be registered in the edge README before activation; never bind a business backend to `0.0.0.0`.
- Only the sole active production container may hold the DingTalk Stream connection for this application credential. Never run production and candidate Stream consumers concurrently with the same Client ID.
