...
- access token 默认 10 分钟；轮换 refresh token 为 30 天滚动窗口。活跃客户端注册会滑动续期，停用 30 天后清理。
...
- 只有 scopes 增加、refresh token 过期或撤销、旧 refresh token 重放导致 family 撤销时，才重新授权。不要用“重新授权”代替普通的断开重连或工具列表刷新。
