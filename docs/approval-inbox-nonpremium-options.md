...
2. 服务端按 OAuth 绑定的当前用户身份（服务端注入，不接受工具参数中的 userId）维护一个受限的 pending 索引；
3. 事件中的 `processInstanceId` 作为审批实例 ID，事件中的任务标识作为 `taskId`；
4. 对索引中的实例再调用现有审批详情/任务接口确认当前状态；
5. `approval_inbox` 只返回已确认的实例和任务 ID，`approval_task` 负责后续查看、同意、拒绝等动作。
...
## 3. `instanceIds/query` + 详情扫描不是可靠替代

流程实例 ID 查询接口能够按审批模板、时间和分页取得实例集合，但它是“实例发现”接口，不是“当前用户待办收件箱”接口。扫描后还必须逐实例拉取详情、读取当前任务执行人并与 OAuth 用户匹配。
...
所以它只能作为事件索引丢失后的“有限重同步”工具：管理员明确提供模板白名单和时间窗口，最大扫描页数固定，结果标记 `resync`，不能宣称与官方 DWS 收件箱等价。
...
有限重同步（可选）
  └─ 仅针对显式配置的 processCode 白名单和最近时间窗口
```

工具返回必须包含：`processInstanceId`、可用时的 `taskId`、`processCode`、任务状态、事件时间、`coverage`、`nextCursor`/`hasMore`。不接受 `userId`、`unionId` 或“代表谁查询”等模型输入；身份来自 MCP OAuth 会话。
...
4. 通过 `approval_inbox(limit=1)` 和批量分页分别验证；
5. 同意/拒绝后确认 `finish/end` 事件移除任务；
